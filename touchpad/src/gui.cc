...
void ParseConnectedInputDevices() {
  ...
  for (UINT deviceIndex = 0; deviceIndex < _numDevices; deviceIndex++) {
    ...
    if (!isButtonCapsEmpty) {
      ...
      auto findInputDeviceRetval = FindInputDeviceInList(g_deviceInfoList, deviceName, cbDeviceName, preparsedData, _prepasedDataSize);
      ...
      if (caps.NumberInputValueCaps != 0) {
        ...
        for (USHORT valueCapIndex = 0; valueCapIndex < numValueCaps; valueCapIndex++) {
          ...
          auto findLinkColRetval                                = FindLinkCollectionInList(g_deviceInfoList.Entries[foundHidIdx].LinkColInfoList, cap.LinkCollection);
          g_deviceInfoList.Entries[foundHidIdx].LinkColInfoList = findLinkColRetval.ModifiedList;
        ...
        for (USHORT buttonCapIndex = 0; buttonCapIndex < numButtonCaps; buttonCapIndex++) {
          ...
          if (buttonCap.UsagePage == HID_USAGE_PAGE_DIGITIZER) {
            if (buttonCap.NotRange.Usage == HID_USAGE_DIGITIZER_TIP_SWITCH) {
              auto findLinkColRetval                                = FindLinkCollectionInList(g_deviceInfoList.Entries[foundHidIdx].LinkColInfoList, buttonCap.LinkCollection);
              g_deviceInfoList.Entries[foundHidIdx].LinkColInfoList = findLinkColRetval.ModifiedList;
              const unsigned int foundLinkColIdx                    = findLinkColRetval.FoundIndex;
...
LRESULT CALLBACK WndProc(_In_ HWND hWnd, _In_ UINT message, _In_ WPARAM wParam, _In_ LPARAM lParam) {
  ...
  switch (message) {
    ...
    case WM_PAINT: {
      ...
      // For this introduction, we just print out "Hello, Windows desktop!" in the top left corner.
