...
struct TOUCHPAD_INFO {
  TCHAR *Name;
  std::vector<COLLECTION_INFO> Collections;
};

static UINT numTouchpads        = 0;
static TOUCHPAD_INFO *touchpads = NULL;
static std::vector<TOUCHPAD_INFO> touchpadDeviceList;
static UINT touchpadDeviceArraySize = 0;
static auto touchpadDeviceArray     = new TOUCHPAD_INFO[touchpadDeviceArraySize];
        ...
        for (UINT i = 0; i < _numDevices; i++) {
          ...
          if (winReturnCode == (UINT)-1) {
            ...
            if (winReturnCode == (UINT)-1) {
              ...
              TOUCHPAD_INFO *touchpadInfoP = NULL;

              std::cout << FG_GREEN << "Finding device in global list..." << RESET_COLOR << std::endl;
              for (std::vector<int>::size_type i = 0; i != touchpadDeviceList.size(); i++) {
                std::cout << "Comparing with ";
                std::wcout << FG_GREEN << touchpadDeviceList[i].Name << RESET_COLOR << std::endl;
                if (_tcscmp(deviceName, touchpadDeviceList[i].Name) == 0) {
                  touchpadInfoP = &(touchpadDeviceList[i]);
                  break;
                }
              }

              if (touchpadInfoP == NULL) {
                std::cout << FG_RED << "Not found in global list!" << RESET_COLOR << std::endl;
                std::vector<COLLECTION_INFO> collectionList;
                TOUCHPAD_INFO touchpadInfo = {deviceName, collectionList};
                touchpadInfoP              = &touchpadInfo;
                touchpadDeviceList.push_back(touchpadInfo);
              } else {
                std::cout << FG_GREEN << "Found device with name " << RESET_COLOR;
                std::wcout << FG_BRIGHT_GREEN << deviceName << RESET_COLOR << std::endl;
              }
