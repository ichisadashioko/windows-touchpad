            ...
            if (isTouchpadRegistered) {
                ...
                if (GetRawInputData((HRAWINPUT)lParam, RID_INPUT, NULL, &rawInputSize, sizeof(RAWINPUTHEADER)) == (UINT)-1) {
                    ...
                    if (lpb == NULL) {
                        ...
                        stdcallReturnValue = GetRawInputData((HRAWINPUT)lParam, RID_INPUT, lpb, &rawInputSize, sizeof(RAWINPUTHEADER));

                        if (stdcallReturnValue == (UINT)-1) {
                            // handle error
                            printTimestamp();
                            printf("Coping RAWINPUT data. GetRawInputData() failed at %s:%d!\n", __FILE__, __LINE__);
                            printLastError();
                        } else if (stdcallReturnValue != _rawInputSize) {
                            printTimestamp();
                            ...
                            if (raw->header.dwType == RIM_TYPEHID) {
                                ...
                                if (count != 0) {
                                    ...
                                    if (GetRawInputDeviceInfo(raw->header.hDevice, RIDI_DEVICENAME, NULL, &deviceNameBufferSize) == (UINT)-1) {
                                        printf("Failed to get device name size at %s:%d!\n", __FILE__, __LINE__);
                                        printLastError();
                                    } else {
                                        printf("deviceNameBufferSize: %d\n", deviceNameBufferSize);
                                        const UINT recordedDeviceNameBufferSize = deviceNameBufferSize;

                                        std::unique_ptr<WCHAR[]> deviceNameBuffer(new WCHAR[deviceNameBufferSize + 1]);
                                        deviceNameBuffer.get()[deviceNameBufferSize] = 0;

                                        stdcallReturnValue = GetRawInputDeviceInfo(raw->header.hDevice, RIDI_DEVICENAME, deviceNameBuffer.get(), &deviceNameBufferSize);
                                        if (stdcallReturnValue == (UINT)-1) {
                                            printf("Failed to get device name at %s:%d\n", __FILE__, __LINE__);
                                            printLastError();
                                        } else if (stdcallReturnValue != recordedDeviceNameBufferSize) {
                                            printf("GetRawInputDeviceInfo does not return the expected size %d (actual) vs %d (expected)\n at %s:%d\n", stdcallReturnValue, recordedDeviceNameBufferSize, __FILE__, __LINE__);
                                        } else {
                                            // TCHAR deviceName = _T(deviceNameBuffer);
                                            // printf("Device name: %s\n", deviceName);
                                            // std::cout << "Device name: " << deviceNameBuffer << std::endl;
                                            WCHAR *deviceName = deviceNameBuffer.get();
                                            std::cout << "Successfully get device name: ";
                                            for (UINT i = 0; i < recordedDeviceNameBufferSize; i++) {
                                                std::wcout << deviceName[i];
                                            }
                                            std::cout << std::endl;
                                        }
                                    }
